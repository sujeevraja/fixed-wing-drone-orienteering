...
class SetCoveringFormulation(private var cplex: IloCplex) {
    ...
    fun createModel(instance: Instance, routes: MutableList<Route>, binary: Boolean = false) {
        ...
        val whichRoutes: MutableList<MutableList<Int>> = MutableList(instance.numTargets) { mutableListOf<Int>() }
     ...
     * @return Dual value
     */
    fun getRouteDual() : Double { return cplex.getDual(constraints[routeConstraintId]) }
    ...
    fun getTargetDuals() : List<Double?> {
        return (0 until hasTargetConstraint.size).map{ if (hasTargetConstraint[it])
